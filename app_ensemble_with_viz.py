...
def create_visualizations(comparison_df, cv_details, ensemble_details, recent_form_details, prediction_year):
    """
    ...
    model_scores = [rf_score, gb_score, lr_score]
    model_weights = [rf_weight, gb_weight, lr_weight]
    
    x_pos = np.arange(len(model_names))
    width = 0.35
    
    bars1 = plt.bar(x_pos - width/2, model_scores, width, label='R² Score', alpha=0.8)
    bars2 = plt.bar(x_pos + width/2, model_weights, width, label='Ensemble Weight', alpha=0.8)
    
    plt.xlabel('Models')
    plt.ylabel('Score / Weight')
    plt.title('Individual Model Performance vs Ensemble Weights')
    ...
    if cv_details:
        ...
        bars = plt.bar(periods, scores, color='skyblue', alpha=0.8)
        plt.ylim(0, 1)
        plt.ylabel('R² Score')
    ...
    plt.hist(errors, bins=range(-10, 11), alpha=0.7, color='skyblue', edgecolor='black')
    ...
    plt.tight_layout()
    plt.savefig(f'{prediction_year}_ensemble_analysis.png', dpi=300, bbox_inches='tight')
    print(f"Visualizations saved as: {prediction_year}_ensemble_analysis.png")
    plt.show()
...
def create_recent_form_trends_plot(data, teams_2024, prediction_year):
    ...
    plt.tight_layout()
    plt.savefig(f'{prediction_year}_form_trends.png', dpi=300, bbox_inches='tight')
    print(f"Form trends plot saved as: {prediction_year}_form_trends.png")
    plt.show()

# Function to Predict for a Specified Year using ensemble approach with enhanced visualization
def compare_predictions(teams, actual_standings, prediction_year):
    ...
    cv_score, cv_details = cross_validate_ensemble_time_series(data, features, target, prediction_year)
    
    print("\n" + "=" * 60)
    print("PREMIER LEAGUE 2024 PREDICTION RESULTS (ENSEMBLE + RECENT FORM + VIZ)")
    print("=" * 60)
    ...
    create_visualizations(comparison_df, cv_details, ensemble_details, recent_form_details, prediction_year)
...
# Call function to compare predictions for 2024 using ensemble approach with visualizations
