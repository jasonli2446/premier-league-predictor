...
def bootstrap_confidence_interval(historical_data, team_data, features, prediction_year, n_bootstrap=1000, confidence_level=0.95):
    """
    ...
    for i in range(n_bootstrap):
        ...
        gb_boot = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, random_state=42)
        ...
        # Calculate weights on a small validation set from bootstrap sample
        val_size = max(20, len(bootstrap_sample) // 10)
        X_val = X_boot.iloc[-val_size:]
        y_val = y_boot.iloc[-val_size:]
        
        rf_val_pred = rf_boot.predict(X_val)
        gb_val_pred = gb_boot.predict(X_val)
        lr_val_pred = lr_boot.predict(X_val)
        
        rf_val_score = max(0.01, r2_score(y_val, rf_val_pred))
        ...
        if len(team_bootstrap) > 0:
            ...
            ensemble_pred, _, _, _ = ensemble_predict(X_team, rf_boot, gb_boot, lr_boot, rf_w, gb_w, lr_w)
...
def compare_predictions_with_confidence(teams, actual_standings, prediction_year, confidence_level=0.95):
    ...
    for i, team in enumerate(teams, 1):
        ...
        ci_results = bootstrap_confidence_interval(
            ...
            n_bootstrap=500, confidence_level=confidence_level  # Reduced for faster execution
        )
